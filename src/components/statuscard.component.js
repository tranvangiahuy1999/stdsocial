...
export default class StatusCard extends React.Component {
  ...
  likeHandle() {
    axiosInstance.put(`/newfeed/like/${this.props.post_id}`).catch((e) => {
      console.error(e);
    });
  }
  ...
  deleteHandle() {
    if (this.props.post_id && this.props.token) {
      axiosInstance
        .delete(`/newfeed/delete/${this.props.post_id}`)
        .then((res) => {
          if (res.data.code === 0) {
            this.setState({
  ...
  handleUpdate() {
    if (
      ...
      (this.state.editytlink && this.state.editytlink.length > 0) ||
  ...
  updateHandle() {
    ...
    let api = `/newfeed/update/${this.props.post_id}`;

    if (this.state.fileInput) {
      if (!this.state.imgcontent) {
        api = `/newfeed/update/new_image/${this.props.post_id}`;
      } else {
        api = `/newfeed/update/image/${this.props.post_id}`;
      }

      formData.append("image", this.state.fileInput);
      formData.append("content", this.state.edittext);
  ...
  render() {
    const menu = (
      <Menu>
        {this.props.user_id &&
          this.props.user_post_id &&
          this.props.role &&
          (this.props.user_id === this.props.user_post_id ||
            this.props.role !== "admin") && (
            <Menu.Item>
              <div style={{ color: "gray" }} onClick={this.showEditModal}>
                <FaRegEdit
                  className="mr-1"
                  style={{ margin: "auto" }}
                  color="gray"
        ...
        <Menu.Item>
          <div style={{ color: "gray" }} onClick={this.showModal}>
            <ImBin
              className="mr-1"
              style={{ margin: "auto" }}
              color="gray"
    ...
    return this.state.deleteStatusState ? (
      ...
      <div className="stc-container bg-white col-12 mt-3 mb-2">
        ...
        <Modal
          title="Update status"
          visible={this.state.editStatusState}
          onOk={this.handleUpdate}
          onCancel={this.handleEditCancel}
        >
          <div className="stp-post" style={{ width: "100%" }}>
            <textarea
              className="post-text p-2"
              rows="3"
            ...
            <div className="stp-preview row ml-2">
              {this.state.ytState && (
                ...
                >
                  <input
                    type="text"
                    value={this.state.editytlink}
                    onChange={(e) =>
                      this.setState({ editytlink: e.target.value })
                    }
              ...
              {this.state.previewFile && (
                <div>
                  ...
                  <ImBin
                    ...
                    color="gray"
                    size="22px"
                    onClick={this.deleteImageHandle}
                  ></ImBin>
                </div>
              )}
            </div>
            <div className="row stp-action">
              <div>
                <MdInsertPhoto
                  ...
                  color="rgba(79,78,75,255)"
                  size="24px"
                  onClick={this.imageBtnHandle}
                ></MdInsertPhoto>
                <input
                  id="update-img"
                  type="file"
        ...
        <div className="stc-header row col-14 pt-2">
          ...
          <div className="col-9" style={{ margin: "auto" }}>
            <div
              className="text-primary username-direct ml-1"
              onClick={this.props.directToWall}
            >
              {this.props.username}
            </div>
            <div className="row ml-0 ml-1">
              <div
                style={{ color: "gray", marginRight: "2px", fontSize: "14px" }}
              >
                {this.props.date}
              </div>
              <AiFillClockCircle
                style={{ margin: "auto", marginLeft: "2px" }}
                size="13px"
                color="gray"
              ></AiFillClockCircle>
            </div>
        ...
        <div className="stc-contain p-1 mt-1" style={{ paddingTop: "4px" }}>
          ...
          {this.state.linkyoutube && this.state.linkyoutube.length > 0 && (
            <div>
              <ReactPlayer
                width="100%"
                controls={true}
                url={this.state.linkyoutube}
                config={{
                  youtube: {
                    playerVars: { origin: "https://youtube.com" },
                  },
                }}
              />
            </div>
          ...
          <div className="row m-2 pt-2">
            <div style={{ width: "50%" }}>
              <div
                style={{ textAlign: "start", color: "gray", fontSize: "15px" }}
              >
            ...
            <div style={{ width: "50%" }}>
              ...
              >
                ...
                {this.state.commentcount > 1 ? " Comments" : " Comment"}
              </div>
            </div>
          </div>
        </div>
        <div className="stc-interact">
          <div className="ml-2 mr-2 row">
            <button
              ...
              style={{ color: this.state.like ? "rgb(0,138,216)" : "gray" }}
            >
              {this.state.like ? (
                <AiFillLike
            ...
            <button onClick={this.cmtHandle} style={{ color: "gray" }}>
              <BiComment
                style={{ margin: "auto", marginRight: "10px" }}
                color="gray"
                size="20px"
          ...
          {this.state.cmtState && (
            <div>
              ...
              <CommentPost
                avatar={this.props.current_avatar}
